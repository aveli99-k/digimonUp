...
던전(3번)은 화면 네 가지만 알아보면 된다.

    challenge  하단의 파란 '도전' 버튼   -> 이걸 누른다
    subjugate  '토벌하기' 버튼           -> 이것도 누른다 (없는 던전이면 안 찍어도 된다)
    fail       위쪽의 '실패...' 글자     -> 졌을 때. 바깥을 눌러 닫는다
    reward     가운데의 '보상' 글자      -> 이겼을 때. 바깥을 눌러 닫는다

앞의 셋과 달리 **subjugate 는 없어도 매크로가 돈다.** 토벌하기가 아예 안 나오는
던전을 도는 사람에게 찍을 수 없는 화면을 요구할 수는 없어서다. 찍어 두면 그때부터
도전과 똑같이 취급한다(둘 다 보이면 유사도가 높은 쪽을 누른다).
...
TARGETS = {
    ...
    "4": ("subjugate", "'토벌하기' 버튼 (없는 던전이면 안 찍어도 됩니다)"),
}
